...
const emptyForm = { nom: '', email: '', password: '', coursId: '', role: 'user' };
...
export default function AdminUtilisateurs() {
  ...
  const openEdit = (u: Utilisateur) => {
    setEditingId(u.id);
    setForm({ nom: u.nom, email: u.email, password: '', coursId: u.coursId ? String(u.coursId) : '', role: u.role ?? 'user' });
    setShowPassword(false);
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        coursId: form.coursId ? parseInt(form.coursId) : null,
        role: form.role || 'user',
      };
      if (editingId !== null) {
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Cours assigné</th>
                    ...
                    <tr key={u.id} className={`border-b border-slate-50 hover:bg-slate-50/70 transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'} ${u.actif === false ? 'opacity-60' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        {u.role === 'admin'
                          ...
                          : <span className="inline-flex items-center bg-green-50 text-green-700 text-xs font-medium px-2.5 py-1 rounded-full">Professeur</span>}
                      </td>
                      <td className="px-4 py-3">
                        {u.actif === false
                      ...
                      <td className="px-4 py-3">
                        {getCoursNom(u.coursId)
                          ? <span className="inline-flex items-center bg-blue-50 text-blue-700 text-xs font-medium px-2.5 py-1 rounded-full">{getCoursNom(u.coursId)}</span>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="user">Professeur</SelectItem>
                  <SelectItem value="admin">Administrateur</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="u-cours">Cours assigné</Label>
              <Select
                value={form.coursId || 'none'}
                onValueChange={v => setForm(f => ({ ...f, coursId: v === 'none' ? '' : v }))}
              >
                <SelectTrigger id="u-cours">
