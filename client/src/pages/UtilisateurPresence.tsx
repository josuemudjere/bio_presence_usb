...
import { loadCourseSettings, saveDepartureException, type CourseSettings, type DepartureReason, type Student } from '@/lib/adminData';
import { fetchStudents, scanAttendanceForCours } from '@/lib/adminApi';
...
function playErrorSound(): void {
  try {
    ...
    // AudioContext indisponible — échec silencieux
  }
}
...
function resolveStudentPhoto(student?: Student, fallbackName?: string): string {
  ...
  return getAvatarUrl(fallbackName ?? student?.name ?? 'Etudiant');
}

function normalizeErrorMessage(input: unknown): string {
  const fallback = 'Empreinte non reconnue.';
  if (!(input instanceof Error) || !input.message) return fallback;
  const lower = input.message.toLowerCase();
  if (lower.includes('aucun etudiant') || lower.includes('non reconnu') || lower.includes('not found')) return fallback;
  return input.message;
}

export default function UtilisateurPresence() {
  const { user } = useAuth();
  const coursId = user?.coursId ?? null;
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      try {
        ...
        if (!mounted) return;
        // Filtrer les étudiants par cours du professeur
        const filtered = coursId != null
          ? apiStudents.filter(s => s.coursId === coursId)
          : apiStudents;
  ...
  const handleScan = async () => {
    ...
    const knownFingerprintIds = students
      .filter(s => s.fingerprintRegistered && s.fingerprintTemplateId)
      .map(s => s.fingerprintTemplateId as string);

    if (knownFingerprintIds.length === 0) {
      setSensorState('error');
      setErrorMessage('Aucune empreinte enregistrée pour ce cours.');
      setResult(null);
      return;
    }
    ...
    try {
      const fingerprintId = await scanFingerprintFromSensor({ mode: 'attendance', knownFingerprintIds });
      setIsListening(false);
      setSensorState('loading');

      if (!isApiReady) {
        const matchedStudent = students.find(s => s.fingerprintTemplateId === fingerprintId);
        ...
        setResult({
          ...
          photoUrl: resolveStudentPhoto(matchedStudent),
          attendanceType: 'entry',
        });
        setSensorState('success');
        speakWelcome(matchedStudent.name);
        return;
      }

      const scanResponse = await scanAttendanceForCours(fingerprintId, coursId);
      const matchedStudent = students.find(s => s.id === scanResponse.attendance.studentId);
      const studentName = matchedStudent?.name ?? scanResponse.attendance.studentName;
  ...
  useEffect(() => {
    if (!autoQueueEnabled || sensorState !== 'idle') return;
          ...
          <div className="w-full max-w-md space-y-6">
            ...
            {connectionState === 'connected' && (
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 flex items-center gap-2">
                ...
                <p className="text-xs font-medium text-emerald-700">Capteur connecté via USB · Prêt à scanner</p>
              </div>
            )}
            ...
            <div className="flex flex-col items-center gap-4">
              <button
                type="button"
                onClick={handleScan}
                disabled={sensorState === 'loading' || connectionState !== 'connected' || !coursId}
              ...
              >
                ...
                {sensorState === 'loading' ? (
                  ...
                  <Fingerprint className={`relative h-24 w-24 ${
                    connectionState !== 'connected' || !coursId ? 'text-slate-300' :
              ...
              <div className="text-center">
                ...
                {sensorState === 'idle' && !isListening && connectionState === 'connected' && coursId && (
                ...
                {sensorState === 'loading' && <p className="text-base font-medium text-emerald-700">Lecture de l'empreinte en cours...</p>}
              ...
              >
                ...
                {!coursId ? 'Aucun cours assigné' : connectionState !== 'connected' ? 'Capteur non connecté' : sensorState === 'loading' ? 'Scan en cours...' : 'Scanner maintenant'}
              </Button>

              {connectionState === 'connected' && coursId && (
                <button
                  type="button"
              ...
              <div className={`rounded-2xl border shadow-md overflow-hidden ${
                ...
                <div className={`flex flex-col items-center gap-3 px-6 py-6 ${
                  ...
                  <div className="text-center">
                    ...
                    <p className="mt-0.5 text-xl font-semibold text-slate-800">{result.studentName}</p>
                    <p className="mt-1 text-sm text-slate-500">{result.matricule} · {result.department}</p>
                  </div>
                  ...
                  {autoQueueEnabled && (
                    <p className="text-xs text-slate-400">Prochain scan automatique dans 2 secondes...</p>
                  )}
                </div>
              </div>
            )}
