...
function normalizeErrorMessage(input: unknown): string {
  ...
  if (lower.includes('aucun etudiant') || lower.includes('non reconnu') || lower.includes('not found')) {
    return fallback;
  }
...
export default function AdminSensor() {
    ...
    try {
      ...
      if (!isApiReady) {
        const matchedStudent = students.find(
          (student) => student.fingerprintTemplateId === fingerprintId
        );
      ...
      const scanResponse = await scanAttendance(fingerprintId);
  ...
  useEffect(() => {
    ...
    if (!autoQueueEnabled || sensorState !== 'idle') {
      return;
    }
